...
/* ── 재는 자들 — 그물을 안 타는 순수 함수. 시험(check-house.test.mjs)이 부른다 ──
   왜 시험이 있나: 이 도구가 곧 이 집의 테스트인데, 도구 자체를 검증하는 것은
   「일부러 깨뜨려 보기」손작업뿐이었다. 그러다 「때말」검사가 제 주석에 속았다
   (아래). 자가 굽었는지는 자로 재야 한다. */
...
export { 뽑, 남은날, 코드만, 사다리 };

/* 시험이 import 로 불렀을 때는 집을 걷지 않는다 — 아래 전부가 이 문 안이다.
   (안쪽을 다시 들여쓰지 않았다: 출력 문자열이 여러 줄이라 들여쓰기가 글을 바꾼다) */
const 스스로 = !!process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href;
if (스스로) {
...
const 방들 = [
  ...
  /* 정치인 조사 버튼 — 주인이 쓰는 도구 쪽이라 색인하지 않고, 제목·표식 규칙도 묻지 않는다 */
  { 이름: "정치 조사",  길: "/politics/",                   색인안함: true, 건너뜀: true },
